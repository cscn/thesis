			...
			for line in infile.readlines():
				...
				if re.match("^#", line):
					...
					if contains_setwd:
						...
						path_to_wd = find_rel_path(contains_setwd.group(1), path)
						if not path_to_wd:
...
def maybe_import_operation(r_command):
	...
	bool
	"""
	r_import_list = ['read', 'load', 'fromJSON', 'source', 'import', 'scan']
				...
				if not re.match('^#', line):
					...
					if potential_path:
						...
						if maybe_import_operation(line):
							...
							file_name = extract_filename(potential_path)
							# try to isolate only the filename portion of the intended path
							if potential_path:
								rel_path = find_rel_path(potential_path, path)
								if rel_path:
									...
									if path_to_file:
										line = re.sub(potential_path, path_to_file, line)
				outfile.write(line)
